...
namespace EditorNew
{
    public class Register : IModule
    {
        ...
        {
            _container.Configure(r => { r.For<object>().Use<MainEditorView>().Named(nameof(MainEditorView)).SetProperty(p => p.DataContext = _container.GetInstance<MainEditorViewModel>()); });
